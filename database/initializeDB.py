...
def createTables(cnx, cursor, db_name):
	try:
		...
		cursor.execute("CREATE TABLE Courses (course_id INT, description VARCHAR(10000), same_as VARCHAR(200), number VARCHAR(50) NOT NULL,name VARCHAR(50) NOT NULL, short_title VARCHAR(100) NOT NULL, department_id INT, PRIMARY KEY (course_id), FOREIGN KEY (department_id) REFERENCES Departments (department_id)) ENGINE = INNODB;")
		...
		cursor.execute("CREATE TABLE Faculty (faculty_id INT AUTO_INCREMENT, first_initial VARCHAR(50) NOT NULL, last_name VARCHAR(300) NOT NULL, full_name VARCHAR(600) NOT NULL, PRIMARY KEY (faculty_id)) ENGINE = INNODB;")
