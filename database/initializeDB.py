...
def createTables(cnx, cursor, db_name):
	try:
		...
		cursor.execute("CREATE TABLE Courses (course_id INT AUTO_INCREMENT, description VARCHAR(10000), same_as VARCHAR(20), number VARCHAR(10) NOT NULL, department_id INT, PRIMARY KEY (course_id), FOREIGN KEY (department_id) REFERENCES Departments (department_id)) ENGINE = INNODB;")
