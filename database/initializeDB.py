...
def createTables(cnx, cursor, db_name):
	try:
		...
		cursor.execute("CREATE TABLE Courses (course_id INT, description VARCHAR(10000), same_as VARCHAR(50), number VARCHAR(50) NOT NULL, department_id INT, PRIMARY KEY (course_id), FOREIGN KEY (department_id) REFERENCES Departments (department_id)) ENGINE = INNODB;")
		...
		cursor.execute("CREATE TABLE Requirements (requirements_id INT AUTO_INCREMENT, details VARCHAR(200), type VARCHAR(3) CHECK (type IN ('CO', 'PRE', 'LAB')), course_id INT, PRIMARY KEY (requirements_id), FOREIGN KEY (course_id) REFERENCES Courses (course_id)) ENGINE = INNODB;")
