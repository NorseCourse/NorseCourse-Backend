...
def createTables(cnx, cursor, db_name):
	try:
		...
		cursor.execute("CREATE TABLE Sections (section_id INT, term VARCHAR(20) CHECK (term IN ('FA', 'JT', 'SP', 'S1', 'S2')), name VARCHAR(50) NOT NULL, short_title VARCHAR(20) NOT NULL, min_credits INT NOT NULL, max_credits INT, comments VARCHAR(500), seven_weeks TINYINT NOT NULL CHECK (seven_weeks >= 0 AND seven_weeks <= 2), course_id INT, PRIMARY KEY (section_id), FOREIGN KEY (course_id) REFERENCES Courses (course_id)) ENGINE = INNODB;")
